...
export function ActionsPanel({
  ...
  hideOnHold: boolean; setHideOnHold: (v: boolean | ((p: boolean) => boolean)) => void;
  taskFilter: TaskFilterTag; setTaskFilter: (v: TaskFilterTag) => void;
  ...
  const selectDate = (mode: DateMode) => {
    setTaskFilter('none');
    setDateMode(mode);
  };

  const selectExtra = (tag: Exclude<TaskFilterTag, 'none'>) => {
    setTaskFilter(tag);
  };

  const dateFilters: { mode: DateMode; label: string }[] = [
    { mode: 'all', label: 'All' },
  ...
  const extraFilters: { tag: Exclude<TaskFilterTag, 'none'>; label: string }[] = [
    { tag: 'priority', label: 'High Priority' },
    { tag: 'someday', label: 'Someday' },
    { tag: 'scheduled', label: 'Scheduled' },
  ];
        ...
        <div className="flex flex-wrap gap-1.5">
          {dateFilters.map(({ mode, label }) => (
            <button
              key={mode}
              type="button"
              onClick={() => selectDate(mode)}
              className={pill(taskFilter === 'none' && dateMode === mode)}
            >
              {label}
            </button>
          ))}
          {extraFilters.map(({ tag, label }) => (
            <button
              key={tag}
              type="button"
              onClick={() => selectExtra(tag)}
              className={pill(taskFilter === tag)}
            >
              {label}
            </button>
          ))}
      ...
      <div>
        ...
        <div className="flex flex-col items-start gap-1.5">
          <button
            ...
            className={pill(!showCompleted)}
          >
            Hide Completed
          </button>
          <button
            type="button"
            onClick={() => setHideOnHold(s => !s)}
            className={pill(hideOnHold)}
          >
            Hide On Hold
          </button>
        </div>
          ...
          return (
            <React.Fragment key={listBlock.id}>
              ...
              {!isUncList && (
                <div
                  ...
                  <button type="button" onClick={() => toggleList(listBlock.id)} className={`w-3 shrink-0 transition-colors ${classes.quickCollapseBtn}`} title={collapsed[listBlock.id] ? 'Expand' : 'Collapse'}>
                    <svg
                      viewBox="0 0 16 16"
                      className={`h-3 w-3 transition-transform ${collapsed[listBlock.id] ? '-rotate-90' : ''}`}
                      fill="none"
                      stroke="currentColor"
                      strokeWidth="1.8"
                    >
              ...
              {tasks.map(task => {
                ...
                if (hideOnHold && task.onHold === true) return null;
                if (taskFilter === 'priority' && !getTaskFlag(task)) return null;
                // someday / scheduled: layout only for now
                  ...
                  <React.Fragment key={task.id}>
                    <div
                      ...
                      <button type="button" onClick={() => handleUpdateBlock(task.id, { checked: !task.checked })}
                        ...
                        {task.checked ? (
                          <span className={`flex h-3.5 w-3.5 items-center justify-center ${classes.quickCheckboxDone}`}>
                          ...
                          <span className={`h-3.5 w-3.5 ${classes.quickCheckbox}`} />
                        )}
                      </button>
